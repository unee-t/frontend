 ...
 */
export function publishClaim (claimId) {
  const { callAPI } = bugzillaApi
  if (!this.userId) return
  const currUser = Meteor.users.findOne({_id: this.userId})
  // Fetching simple data
  callAPI('get', `/rest/bug/${claimId}`, {token: currUser.bugzillaCreds.token})
    .then(data => {
      this.added('claims', claimId, data.bugs[0])
      this.ready()
    })
    .catch(err => {
...
if (Meteor.isServer) {
  Meteor.publish('claim', publishClaim)
}
let Claims
if (Meteor.isClient) {
